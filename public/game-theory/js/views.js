...
(function () {
  ...
  function topbar(active) {
    ...
    return '<header class="topbar">' +
      '<div class="brand" data-nav="home"><div class="owl">♟️</div><span>博弈思维</span></div>' +
      '<nav>' + nav.map(n =>
      ...
      '<button class="avatar-btn" data-nav="profile" title="' + u.username + '">' + (u.equipped && u.equipped.avatar ? u.equipped.avatar : '♟️') + '</button>' +
      '</div></header>';
  }
  ...
  function go(view) {
    ...
    if (tbEl) {
      ...
      if (av) av.textContent = (u.equipped && u.equipped.avatar ? u.equipped.avatar : '♟️');
  ...
  function renderAuth(mode) {
    ...
    app.innerHTML = '<div class="auth-wrap"><div class="auth-card">' +
      '<div class="owl-big">♟️</div>' +
      '<h1>博弈思维</h1>' +
      '<p class="tag">用多邻国的方式，学完《用博弈的思维看世界》' + Curriculum.UNITS.length + ' 个章节的每一个知识点。<br>看懂对手，赢得共同利益。</p>' +
      err + fields +
  ...
  function renderHome() {
    ...
    Curriculum.UNITS.forEach(unit => {
      ...
      unit.sections.forEach((s, i) => {
        ...
        sectHtml += '<div class="' + nodeClass + '">' +
          ...
          '<div class="node-desc">' + (done ? '已完成 · ' + (rec.best || 0) + ' XP' : '先学后测，掌握博弈思维') + '</div>' +
          '</div>' +
      ...
      const ringColor = unitDone ? part.color : '#cbd5e1';
    ...
    shell.innerHTML = '<div class="view">' + xpBar +
      heroBanner(u, full) +
      '<div class="banner-msg win" style="background:#e5f5e1"><b>今日目标：</b>' + u.plan.dailyTarget + ' XP · 从下面开始今天的博弈课。</div>' +
      unitsHtml +
      ...
      '<button class="btn btn-outline" data-go="sim">🤖 博弈模拟器</button> ' +
      '</div></div>';
  ...
  function heroBanner(u, full) {
    ...
    return '<div class="hero-banner">' +
      ...
      '<h1>用博弈的思维看世界</h1>' +
      '<div class="hero-sub">蒋文华 · ' + full.length + '/' + Curriculum.UNITS.length + ' 个章节 · 全书进度 ' + pct + '%</div>' +
  ...
  function renderSimHub() {
    ...
    const shell = el('shell');
    const sims = [
      { id: 'prisoner', icon: '⛓️', name: '囚犯困境', desc: '坦白还是抗拒？看穿个体理性与集体理性的冲突', tag: '经典模型' },
      { id: 'auction', icon: '💵', name: '万元陷阱', desc: '美元拍卖实战，体验越陷越深的滋味', tag: '人性陷阱' },
      { id: 'boxedpigs', icon: '🐷', name: '智猪博弈', desc: '当小猪，学搭便车；看大猪，懂责任', tag: '搭便车' },
      { id: 'chicken', icon: '🐓', name: '懦夫博弈', desc: '冲还是退？直面狭路相逢的抉择', tag: '对抗' },
      { id: 'battle', icon: '💑', name: '性别战', desc: '一起看什么节目？合作从妥协开始', tag: '合作' },
      { id: 'ultimatum', icon: '⚖️', name: '最后通牒', desc: '你来分100元，对方能拒绝', tag: '公平' },
      { id: 'repeated', icon: '🔁', name: '重复博弈', desc: '和AI玩20轮，体会一报还一报的威力', tag: '长期合作' },
    ];
    shell.innerHTML = '<div class="view"><h2 style="margin-bottom:6px">🤖 博弈模拟器</h2>' +
      '<p class="muted" style="margin-bottom:16px">亲手玩一局，比背一百遍更懂博弈。每种模拟都和书中案例一一对应。</p>' +
      '<div class="games-grid">' + sims.map(s =>
        '<div class="game-card" data-sim="' + s.id + '"><div class="icon">' + s.icon + '</div>' +
        '<h4>' + s.name + '</h4><p>' + s.desc + '</p><span class="tag green" style="margin-top:8px">' + s.tag + '</span></div>'
      ).join('') + '</div>' +
      '<div class="card" style="margin-top:20px"><h3>🏅 模拟战绩</h3>' +
      '<div class="grid-3">' + statCard('🤖', u.simsPlayed || 0, '参与场次') + statCard('🏆', u.simsWon || 0, '获胜场次') + statCard('🪙', u.gems, '宝石') + '</div></div>' +
      '</div>';
    shell.querySelectorAll('[data-sim]').forEach(b => b.addEventListener('click', () => {
      SFX.play('click');
      window.APP.launchSim(b.getAttribute('data-sim'));
    }));
  }
  ...
  function renderStats() {
    ...
    shell.innerHTML = '<div class="view">' +
      ...
      statCard('🏁', full.length + '/' + Curriculum.UNITS.length, '已完成章节') +
      statCard('🎯', acc + '%', '测验准确率') +
  ...
  function calendarHtml(u) {
    ...
    for (let i = 0; i < 28; i++) {
      ...
      cells += '<div class="day ' + (isOn ? 'on' : '') + ' ' + (isToday ? 'today' : '') + '">' + (isOn ? '✓' : d.getDate()) + '</div>';
    }
    return cells + '</div>';
  }
  ...
  function dailyRows(tasks, done) {
    return tasks.map(t => {
      ...
      return '<div class="task-row ' + (t.cur >= t.goal ? 'done' : '') + '">' +
        ...
        '</div>';
    }).join('') + '<div class="muted" style="font-size:12px;margin-top:6px">今天完成 ' + done + '/' + tasks.length + '</div>';
  }
  ...
  function reportSummaryHtml(u) {
    ...
    return '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;font-size:14px">' +
      summaryRow('完成的章节', full.length + ' / ' + Curriculum.UNITS.length) +
  ...
  function summaryRow(k, v) {
    return '<div style="background:var(--bg-2);border-radius:10px;padding:10px 14px"><div class="muted" style="font-size:12px">' + k + '</div><b>' + v + '</b></div>';
  }
  ...
  /* ---------------- 商城 ---------------- */
  const SHOP_ITEMS = [
    { id: 'avatar_king', icon: '👑', name: '博弈之王', desc: '王者风范的棋子头像', price: 200, type: 'avatar' },
    { id: 'avatar_fox', icon: '🦊', name: '机灵狐狸', desc: '像狐狸一样聪明的头像', price: 250, type: 'avatar' },
    { id: 'avatar_dragon', icon: '🐉', name: '巨龙', desc: '威风凛凛的龙头像', price: 400, type: 'avatar' },
    { id: 'avatar_owl', icon: '🦉', name: '智慧猫头鹰', desc: '深夜思考的猫头鹰头像', price: 350, type: 'avatar' },
    ...
    { id: 'title_thinker', icon: '🎓', name: '策略思考者称号', desc: '获得"策略思考者"称号', price: 600, type: 'title', value: '策略思考者' },
    { id: 'title_master', icon: '👑', name: '博弈大师称号', desc: '获得"博弈大师"称号', price: 1000, type: 'title', value: '博弈大师' },
  ...
  function renderCommunity() {
    ...
    shell.innerHTML = '<div class="view"><h2 style="margin-bottom:6px">💬 学习社区</h2>' +
      ...
      '<div class="post-input"><input id="post-text" placeholder="分享一点关于博弈思维的心得…"><button class="btn btn-green" id="post-send">发布</button></div>' +
      '<div id="post-list">' + posts.map(p => postHtml(p, u)).join('') + '</div>' +
      (posts.length ? '' : '<div class="empty-state"><div class="big">♟️</div><p>成为第一个发帖的人吧！</p></div>') +
      '</div>';
  ...
  function renderCertificate() {
    ...
    const title = u.equipped && u.equipped.title ? u.equipped.title : '博弈学习者';
    ...
    shell.innerHTML = '<div class="view">' +
      (done ? '<div class="certificate"><div class="stamp">已完成</div>' +
        ...
        '<p class="congrats">已完成《用博弈的思维看世界》全部 ' + Curriculum.UNITS.length + ' 个章节、' + Curriculum.totalSections + ' 个小节的学习。<br><b>用博弈的思维看世界</b><br>蒋文华 著</p>' +
      ...
      : '<div class="empty-state"><div class="big">🎖️</div><h2 style="margin:10px 0">还差一步！</h2>' +
        '<p>完成全部 ' + Curriculum.UNITS.length + ' 个章节就能获得证书。你已经完成了 <b>' + full.length + ' / ' + Curriculum.UNITS.length + '</b> 个章节。</p>' +
  ...
  function renderProfile() {
    ...
    shell.innerHTML = '<div class="view"><div class="two-col">' +
      ...
      '<div style="font-size:70px">' + (u.equipped.avatar || '♟️') + '</div>' +
  ...
  function renderGames() {
    ...
    shell.innerHTML = '<div class="view"><h2 style="margin-bottom:6px">🎮 学习游戏</h2>' +
      ...
      gameCard('flashcard', '🃏', '翻卡记忆', '复习博弈术语，每赢一局 +5 XP') +
  ...
  function renderReport() {
    ...
    shell.innerHTML = '<div class="view"><h2 style="margin-bottom:6px">📊 学习报告</h2>' +
      ...
      '</div>' +
      '<div class="card"><h3>📈 逐章节进度</h3>' + perChapter + '</div>' +
  ...
  function renderSettings() {
    ...
    el('set-dark').addEventListener('change', e => {
      ...
      localStorage.setItem('gt_dark', e.target.checked ? '1' : '0');
      SFX.play('click');
    });
    el('export-data').addEventListener('click', () => {
      ...
      a.download = '博弈思维-备份-' + new Date().toISOString().slice(0, 10) + '.json';
      a.click();
    ...
    el('reset-data').addEventListener('click', () => {
      if (confirm('确定要重置全部数据吗？此操作不可恢复。')) {
        Store.resetAll();
        localStorage.removeItem('gt_dark');
        localStorage.removeItem('gt_muted');
        localStorage.removeItem('gt_music');
        SFX.play('click');
