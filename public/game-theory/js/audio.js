/* ============================================================
   博弈思维 — audio.js
   Web Audio API 实时合成音效（无需外部文件）。
   ============================================================ */
(function () {
  'use strict';

  let ctx = null;
  let muted = localStorage.getItem('gt_muted') === '1';
  let musicOn = localStorage.getItem('gt_music') === '1';
  let musicTimer = null;
  let musicStep = 0;
  ...
  const S = {
    ...
    // 博弈模拟器专用音效
    win: function () { [523, 659, 784, 1046].forEach((f, i) => tone(f, 0.18, 'triangle', 0.2, i * 0.09)); },
    lose: function () { tone(330, 0.2, 'sawtooth', 0.14, 0, 180); tone(220, 0.3, 'sawtooth', 0.14, 0.15, 110); },
    countdown: function () { tone(440, 0.06, 'square', 0.1); },
    reveal: function () { tone(700, 0.1, 'triangle', 0.16); tone(1050, 0.2, 'triangle', 0.16, 0.08); },
    deal: function () { tone(320, 0.08, 'square', 0.12); tone(480, 0.12, 'square', 0.12, 0.06); },
  };
  ...
  function startMusic() {
    ...
    musicOn = true;
    localStorage.setItem('gt_music', '1');
    musicStep = 0;
  ...
  function stopMusic() {
    musicOn = false;
    localStorage.setItem('gt_music', '0');
    if (musicTimer) { clearInterval(musicTimer); musicTimer = null; }
  }

  const API = {
    play: function (name, arg) { if (S[name]) S[name](arg); },
    setMuted: function (m) { muted = m; localStorage.setItem('gt_muted', m ? '1' : '0'); if (m) stopMusic(); },
  ...
  document.addEventListener('pointerdown', function once() {
    API.unlock();
    if (localStorage.getItem('gt_music') === '1') startMusic();
