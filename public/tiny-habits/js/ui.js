/* ============================================================
   Tiny Habits — ui.js
   共享 UI 助手：提示、庆祝、彩带、XP/等级数学、成就引擎、
   每日任务、排行榜。
   ============================================================ */
(function () {
  'use strict';

  const LEVEL_XP = 100; // 升一级所需 XP
  const ACHIEVEMENTS = [
    { id: 'first_lesson', icon: '🌱', name: '播种者', desc: '完成第一节课', check: u => u.lessonsCompleted >= 1 },
    { id: 'five_lessons', icon: '📚', name: '小书虫', desc: '完成 5 节课', check: u => u.lessonsCompleted >= 5 },
    { id: 'twenty_five', icon: '📖', name: '行为学者', desc: '完成 25 节课', check: u => u.lessonsCompleted >= 25 },
    { id: 'chapter_complete', icon: '🏁', name: '章末冲刺', desc: '完成第一个完整单元（章）', check: u => fullChapters(u).length >= 1 },
    { id: 'half_book', icon: '📗', name: '半程微习惯', desc: '完成全书一半的单元', check: u => fullChapters(u).length >= 4 },
    ...
    { id: 'streak_30', icon: '🌋', name: '习惯大师', desc: '连续打卡 30 天', check: u => u.bestStreak >= 30 },
    { id: 'level_5', icon: '⭐', name: '新星', desc: '达到 5 级', check: u => u.level >= 5 },
    { id: 'level_10', icon: '💎', name: '钻石头脑', desc: '达到 10 级', check: u => u.level >= 10 },
    ...
    { id: 'garden_1', icon: '🌻', name: '第一颗种子', desc: '在习惯花园种下第 1 个习惯', check: u => (u.habits && u.habits.length) >= 1 },
    { id: 'garden_5', icon: '🌳', name: '园丁', desc: '在习惯花园种下 5 个习惯', check: u => (u.habits && u.habits.length) >= 5 },
    { id: 'maui_7', icon: '🌅', name: '晨曦之子', desc: '连续 7 天完成 Maui 习惯', check: u => (u.bestMauiStreak || 0) >= 7 },
    { id: 'full_book', icon: '👑', name: '微小之王', desc: '完成全书的每一个小节', check: u => fullChapters(u).length >= 8 },
  ];
  ...
  function getDailyTasks(u) {
    ...
    return [
      ...
      { id: 'quiz', name: '在测验中得满分', goal: 1, cur: p.quiz, reward: 5 },
    ];
  }
